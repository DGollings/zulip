...
class OnedevHookTests(WebhookTestCase):
    STREAM_NAME = "commits"
    URL_TEMPLATE = "/api/v1/external/onedev?&api_key={api_key}&stream={stream}"
    WEBHOOK_DIR_NAME = "onedev"
...
#         self.check_webhook("create__branch", expected_topic, expected_message)
    ...
    def test_pull_request_opened(self) -> None:
        expected_topic = "test / PR #28 Title Text for Pull Request"
        expected_message = """daniel opened PR #28 from `feature` to `master`."""
    ...
    def test_pull_request_closed(self) -> None:
        ...
        expected_message = """daniel discarded PR #28 from `feature` to `master`."""
        self.check_webhook("pull_request__discarded", expected_topic, expected_message)

    def test_pull_request_merged(self) -> None:
        expected_topic = "test / PR #28 Title Text for Pull Request"
        expected_message = """daniel merged PR #28 from `feature` to `master`."""
        self.check_webhook("pull_request__merged", expected_topic, expected_message)

    def test_pull_request_reopened(self) -> None:
        expected_topic = "test / PR #28 Title Text for Pull Request"
        expected_message = """daniel reopened PR #28 from `feature` to `master`."""
...
#         self.check_webhook("pull_request__synchronized", expected_topic, expected_message)
    ...
    def test_issues_opened(self) -> None:
        ...
        expected_message = """testuser opened issue #3 New test issue:\n\n~~~ quote\nlorem ipsum dolor\n~~~"""
        self.check_webhook("issue__opened", expected_topic, expected_message)

    def test_issues_changed(self) -> None:
        expected_topic = "test / issue #3 New test issue"
        expected_message = """testuser changed issue #3 New test issue:\n\n~~~ quote\ntestuser changed 'state' from 'Open' to 'Closed'\n~~~"""
        self.check_webhook("issue__changed", expected_topic, expected_message)
